...
class ImportElements extends \App\Controllers\BaseController {
	...
	public function index(){
		$data = array();
		$this->load->view('Generated/element/importelements_view', $data);
	}
	...
	private function parseFile($filename, $separator){
		...
		if (($handle = fopen($filename, "r")) !== FALSE) {
			while (($data = fgetcsv($handle, $maxSize, $separator)) !== FALSE) {
				...
				$model->name = $data[0];
				$model->selector = $data[1];
				$model->page_id = $data[2];	
				$model->save($this->db);
